...
<html>
    ...
    <body>
		...
		<div id=center>
                    ...
                    <div id=right>
                        <c:set var="prds" value="${ResultSet}"/>
                        <p id=bestseller>Kết quả tìm kiếm: </p>
                        <br>
                        <c:choose>
                            <c:when test="${not empty prds}">
                                <c:forEach var="rows" items="${prds}">
                                    <div class="product">
                                        <form action="SBuy">
                                            <img class=pic src="${rows.anh}" alt="">
                                            <br>
                                            <p style="padding-bottom: 10px;" class="tit">${rows.tensp}</p>
                                            <label style="margin-left: 80px;margin-right: 5px;">Đơn giá:</label>${rows.gia}đ<br><br/>
                                            <label style="margin-left: 80px;margin-right: 5px;">Số lượng:</label>
                                            <input style="text-align: center;width: 50px;" type="number" min="1" name="txtNumber" value="1"/><br/><br/>
                                            <input type="hidden" name="hiddenKey" value="${rows.masp}"/>
                                            <input type="hidden" name="hiddenName" value="${rows.tensp}"/>
                                            <input type="hidden" name="hiddenType" value="${rows.loaisp}"/>
                                            <input type="hidden" name="hiddenPrice" value="${rows.gia}"/>
                                            <input type="hidden" name="hiddenUrl" value="${rows.anh}"/>                                     
                            ...
                            <c:otherwise>
                                <p id="none">Không tìm thấy sản phẩm!</p>
                            </c:otherwise>
                        </c:choose>                                  
                    </div>
		</div>
